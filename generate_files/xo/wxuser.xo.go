...
// WxUser represents a row from 'aypcddg.wx_users'.
type WxUser struct {
	ID         int            `json:"id"`          // id
...
func GetWxUserTableName(key ...interface{}) (string, error) {
	tableName, err := components.M.GetTable("aypcddg", "wx_users", key...)
	if err != nil {
		return "", err
	}
...
// WxUserByAppidOpenid retrieves a row from 'aypcddg.wx_users' as a WxUser.
//
...
// WxUsersByAppidUnionid retrieves a row from 'aypcddg.wx_users' as a WxUser.
//
// Generated from index 'idx_appid_unionid'.
func WxUsersByAppidUnionid(ctx context.Context, appid sql.NullString, unionid sql.NullString, key ...interface{}) ([]*WxUser, error) {
	var err error
	var dbConn *sql.DB
	...
	sqlstr := `SELECT ` +
		`id, openid, session_id, unionid, appid, uid, gcode, tcode, tuid, tfid, tsid, session_key, expires_in, nickName, gender, avatarUrl, city, province, country, created, updated ` +
		`FROM ` + tableName +
		` WHERE appid = ? AND unionid = ?`

	// run query
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, appid, unionid)))
	...
	if tx != nil {
		queryData, err = tx.Query(sqlstr, appid, unionid)
		if err != nil {
			return nil, err
		}
	} else {
		queryData, err = dbConn.Query(sqlstr, appid, unionid)
		if err != nil {
			return nil, err
		}
	}
...
// WxUserByID retrieves a row from 'aypcddg.wx_users' as a WxUser.
//
